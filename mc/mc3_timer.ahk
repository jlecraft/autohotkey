...
; ----------------------------
; Fishing session timer (starts on Ctrl+F)
; ----------------------------
global FishingStartTick := 0   ; A_TickCount when timer started
global FishingTimerRunning := false
...
UpdateUI() {
    global lblState, sb
    global CurrentState, FishingAvgMs, FishingIntervals, FishingCasts, FishingStartTick
    ...
    if IsSet(sb) && sb {
        if (CurrentState = "Fishing") {
            ...
            timeText := (FishingStartTick ? FormatElapsed(A_TickCount - FishingStartTick) : "00:00:00")
            sb.SetText("State: " CurrentState " | Time: " timeText " | Avg: " avgText " (" FishingCasts ")")
        } else {
...
; ----------------------------
; Fishing session timer helpers
; ----------------------------
StartFishingTimer() {
    global FishingStartTick, FishingTimerRunning
    FishingStartTick := A_TickCount
    FishingTimerRunning := true
    ; refresh immediately + then every second
    UpdateUI()
    SetTimer UpdateFishingTimer, 1000
}

StopFishingTimer() {
    global FishingTimerRunning
    FishingTimerRunning := false
    SetTimer UpdateFishingTimer, 0
}

UpdateFishingTimer() {
    ; just repaints the status bar so the elapsed time stays current
    if !IsState("Fishing")
        return
    UpdateUI()
}
...
OnStateChanged() {
    ...
    ; Keep the fishing timer from running in other modes
    if (GetState() != "Fishing")
        StopFishingTimer()
